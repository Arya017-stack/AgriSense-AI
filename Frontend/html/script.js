const API_BASE = (location.hostname === "127.0.0.1" || location.hostname === "localhost")
  ...
  // ===== HERO DASHBOARD =====
      ...
      const heroCropLabel = document.querySelector("#heroSugarcane").closest(".mini-card").querySelector("h3");
      
      if (heroCropLabel) {
              heroCropLabel.innerText = "🌾 " + (data.top_crop || "Crop");
          }
...
async function loadHistory() {
  try {
    ...
    receipts.forEach((receipt) => {
      const isManual = receipt.is_manual_entry === 1;
      const proofCell = isManual
        ? '—'
        : `<a href="${API_BASE}/uploads/${receipt.receipt_name}" target="_blank" style="color:#4ade80;">📄 View</a>`;
      ...
      fetch(`${API_BASE}/weather?lat=${lat}&lon=${lon}`)
        ...
        .then(data => {
          if (data.error) {
            ...
            document.getElementById('cropSuggestionReason').innerText = '';
        ...
        const seasonal = data.seasonal_recommendation;
        if (seasonal) {
          const statusText = seasonal.status === "sow_now"
            ? `✅ Ab bo sakte hain: ${seasonal.recommended_crop}`
            : seasonal.status === "upcoming"
              ? `⏳ ${seasonal.recommended_crop} ka season ${seasonal.days_until_sowing} din mein aayega`
              : `😐 Filhaal koi major sowing window nahi`;

          document.getElementById('cropSuggestion').innerText = statusText;
          document.getElementById('cropSuggestionReason').innerText = seasonal.reason;
        } else {
  ...
  async function loadForecastAccuracy() {
    try {
      ...
      el.innerText = `±${data.mae_temp}°C avg error (${data.sample_size} days)`;
    } catch (err) {
